...
namespace azima\Laravel_paytabs\Services;
...
class IpnRequest
{
    ...
    private function isValidIPNRequest($httpRequest){
        ...
        new \azima\Laravel_paytabs\paytabs_core(); //this is a hack just to be able to use `PaytabsApi` class from paytabs_core.php !!!

        $paytabs_api= \azima\Laravel_paytabs\PaytabsApi::getInstance(config('paytabs.region'), config('paytabs.profile_id'), config('paytabs.server_key'));
